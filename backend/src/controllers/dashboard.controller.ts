...
export const getDashboardStats = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (role === 'super_admin') {
      ...
      const [tenantsRes, studentsRes, classesRes, teachersRes, activitiesRes, attendanceRateRes] = await Promise.all([
        ...
        `),
        pool.query(`SELECT ROUND(AVG(CASE WHEN status = 'present' THEN 100 ELSE 0 END)) as rate FROM attendance WHERE created_at > CURRENT_DATE - INTERVAL '30 days'`)
      ]);
    ...
    const limitsRes = planId
      ...
      : { rows: [] as any[] };
    const limits: Record<string, number> = (limitsRes.rows as any[]).reduce((acc: any, row: any) => {
    ...
    res.json({
      ...
      attendanceTrends: attendanceRes.rows.length > 0 ? attendanceRes.rows : [
