...
export class DashboardService {
  static async getDashboardStats(
    ...
    if (role === ROLES.SUPER_ADMIN && !headers[HTTP_HEADERS.TENANT_ID]) {
      ...
      const [tenantsRes, studentsRes, classesRes, teachersRes, activitiesRes, attendanceRateRes] =
        await Promise.all([
          ...
          pool.query(`
          (SELECT 'student' as type, full_name as user, 'STUDENT_REGISTERED' as action, '' as target, created_at FROM students ORDER BY created_at DESC LIMIT 5)
          UNION ALL
          (SELECT 'tenant' as type, name as user, 'TENANT_JOINED' as action, '' as target, created_at FROM tenants ORDER BY created_at DESC LIMIT 5)
    ...
    if (role === 'teacher') {
      let email = null; // Assuming teacher details passed via logic if needed
      if (!email && userId) {
      ...
      if (teacherRes.rows.length === 0) {
        return { activeClasses: 0, upcomingSessions: 0, students: 0 };
      }
    ...
    if (role === 'student') {
      ...
      if (studentRes.rows.length === 0) {
        return { enrolledClasses: 0, upcomingSessions: 0 };
      }
    ...
    const period = (query.period as string) === 'yearly' ? 12 : 6;
    const [
      totalClassesRes,
      classesRes,
      teachersRes,
      ...
      pool.query(
        `
        (SELECT 'student' as type, full_name as user, 'STUDENT_REGISTERED' as action, '' as target, created_at FROM students WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 3)
        UNION ALL
        (SELECT 'class' as type, name as user, 'CLASS_CREATED' as action, '' as target, created_at FROM classes WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 2)
  ...
  static async getActivities(
    ...
    if (role === 'teacher') {
      ...
      if (teacherRes.rows.length === 0)
        return { activities: [], pagination: { total: 0, page, limit, totalPages: 0 } };
      const teacherId = teacherRes.rows[0].id;

      // Teachers only see students enrolled in their classes and their own class creation (if any)
      sql = `
        SELECT * FROM (
          (SELECT 'student' as type, s.full_name as user, 'STUDENT_JOINED_CLASS' as action, c.name as target, cs.enrolled_at as created_at 
           FROM class_students cs 
           JOIN students s ON cs.student_id = s.id 
           JOIN classes c ON cs.class_id = c.id 
           WHERE c.teacher_id = $1 AND c.tenant_id = $2 AND s.is_deleted = false)
          UNION ALL
          (SELECT 'class' as type, name as user, 'CLASS_ASSIGNED' as action, '' as target, created_at 
      ...
      sql = `
        SELECT * FROM (
          (SELECT 'student' as type, full_name as user, 'STUDENT_REGISTERED' as action, '' as target, created_at FROM students WHERE tenant_id = $1 AND is_deleted = false)
          UNION ALL
          (SELECT 'class' as type, name as user, 'CLASS_CREATED' as action, '' as target, created_at FROM classes WHERE tenant_id = $1 AND is_deleted = false)
          UNION ALL
          (SELECT 'teacher' as type, full_name as user, 'TEACHER_JOINED' as action, '' as target, created_at FROM teachers WHERE tenant_id = $1 AND is_deleted = false)
        ) combined
        ORDER BY created_at DESC
        LIMIT $2 OFFSET $3
      `;
    ...
    const [activitiesRes, countRes] = await Promise.all([
      ...
      pool.query(
        countSql,
        [role === 'teacher' ? params[0] : params[0], tenantId].slice(0, role === 'teacher' ? 2 : 1)
      ),
    ]);

    const countParams = role === 'teacher' ? [params[0], params[1]] : [params[0]];
