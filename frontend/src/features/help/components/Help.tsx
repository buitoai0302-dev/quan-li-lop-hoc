  ...
  const allCategories = [
    {
      ...
      details:
        user?.role === USER_ROLES.TEACHER
          ? t('help.teacher.gettingStarted.details')
          : t('help.categories.gettingStarted.details'),
    },
    {
      id: 'scheduling',
      ...
      title:
        user?.role === USER_ROLES.TEACHER
          ...
          : t('help.categories.scheduling.title'),
      description:
        user?.role === USER_ROLES.TEACHER
          ? t('help.teacher.scheduling.desc')
          : t('help.categories.scheduling.desc'),
      steps:
        user?.role === USER_ROLES.TEACHER
          ? [
      ...
      details:
        user?.role === USER_ROLES.TEACHER
          ? t('help.teacher.scheduling.details')
          : t('help.categories.scheduling.details'),
    },
    {
      id: 'data-management',
      ...
      steps: [t('help.categories.data.step1'), t('help.categories.data.step2')],
      ...
      <div className="max-w-6xl mx-auto space-y-8 pb-12">
        ...
        <div className="bg-gradient-to-r from-primary to-indigo-600 rounded-2xl p-8 md:p-12 text-white shadow-xl relative overflow-hidden">
          <div className="relative z-10 max-w-2xl">
            ...
            <p className="text-lg text-white/80 leading-relaxed">{t('help.subtitle')}</p>
          </div>
              ...
              <div className="p-8 overflow-y-auto flex-1 space-y-6">
                ...
                <div className="pt-6 border-t border-gray-100 dark:border-gray-700">
                  <h4 className="font-bold text-gray-900 dark:text-white mb-4 text-sm uppercase tracking-wider">
                    {t('help.stepsToFollow')}
                  </h4>
                  <div className="space-y-3">
                    {selectedCategory.steps.map((step: string, i: number) => (
                      <div
                        key={i}
                        className="flex gap-4 p-3 bg-gray-50 dark:bg-gray-700/30 rounded-lg"
                      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              <h4 className="font-bold text-gray-800 dark:text-gray-200">
                {t('help.quickTips.tip1.q')}
              </h4>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {t('help.quickTips.tip1.a')}
              </p>
            </div>
            <div className="space-y-2">
              <h4 className="font-bold text-gray-800 dark:text-gray-200">
                {t('help.quickTips.tip2.q')}
              </h4>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {t('help.quickTips.tip2.a')}
              </p>
            </div>
            <div className="space-y-2">
              <h4 className="font-bold text-gray-800 dark:text-gray-200">
                {user?.role === USER_ROLES.TEACHER
                  ? t('help.quickTips.teacher.tip3.q')
                  : t('help.quickTips.tip3.q')}
              </h4>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {user?.role === USER_ROLES.TEACHER
                  ? t('help.quickTips.teacher.tip3.a')
                  : t('help.quickTips.tip3.a')}
              </p>
            </div>
          </div>
        </div>
