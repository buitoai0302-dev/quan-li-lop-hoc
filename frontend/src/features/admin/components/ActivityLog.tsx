...
const ActivityLog: React.FC = () => {
  ...
  const getActivityIcon = (type: string) => {
    switch (type) {
      ...
      case 'teacher':
        return <UserCheck size={18} className="text-amber-500" />;
      case 'session':
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-1 w-full overflow-hidden bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 flex flex-col min-h-0">
        {loading ? (
          ...
          <>
            <div className="flex-1 overflow-y-auto custom-scrollbar divide-y divide-gray-50 dark:divide-gray-700/50">
              {activities.map((act) => (
                <div
                  key={act.id}
                  className="p-3 sm:p-5 hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors flex gap-3 sm:gap-4 group"
                >
          ...
          <EmptyState title={t('dashboard.noActivity')} icon={Activity} />
        )}
      </div>
    </div>
  );
