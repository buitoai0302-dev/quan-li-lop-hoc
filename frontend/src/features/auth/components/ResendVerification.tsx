      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-[440px] relative z-10">
        <div className="bg-white/70 dark:bg-gray-800/50 backdrop-blur-xl py-8 px-6 sm:px-10 shadow-2xl shadow-gray-200/50 dark:shadow-none rounded-[2rem] sm:rounded-[2.5rem] border border-white dark:border-gray-700/50">
          {sent ? (
            <div className="text-center space-y-6">
              ...
              <div>
                <h3 className="text-xl font-black text-gray-900 dark:text-white mb-2">
                  {t('auth.resendSuccessTitle')}
                </h3>
                <p className="text-gray-500 dark:text-gray-400 text-xs leading-relaxed">
                  {t('auth.resendSuccessDesc', { email })}
                </p>
              </div>
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              <div className="space-y-1.5">
                <label
                  htmlFor="email"
                  className="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest ml-1"
                >
                  {t('auth.email')}
                </label>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <Mail
                      size={18}
                      className="text-gray-300 group-focus-within:text-primary transition-colors"
                    />
                  </div>
                  <input
                    id="email"
          ...
          <div className="mt-8 text-center pt-6 border-t border-gray-50 dark:border-gray-700/50">
            <Link
              to="/login"
              className="inline-flex items-center justify-center gap-2 text-[11px] font-black uppercase tracking-widest text-gray-400 hover:text-primary transition-all"
            >
