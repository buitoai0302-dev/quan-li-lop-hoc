...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, user, onLogout }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`fixed md:static inset-y-0 left-0 z-[70] w-60 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
              ...
              {({ isActive }) => (
                <>
                  ...
                  {item.isPremium && ['admin', 'staff', 'super_admin'].includes(user?.role || '') && (
                    <div className="flex items-center justify-center px-1.5 py-0.5 bg-gradient-to-tr from-amber-400 to-orange-500 rounded-md shadow-lg shadow-orange-500/20 animate-pulse">
        ...
        <div className="p-1 border-t border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between gap-3 bg-gray-50 dark:bg-gray-900/50 p-2 rounded-2xl border border-gray-100 dark:border-gray-800 mb-2">
