        ...
        <div className="bg-white/70 dark:bg-gray-800/50 backdrop-blur-xl py-12 px-6 sm:px-10 shadow-2xl shadow-gray-200/50 dark:shadow-none rounded-[2rem] sm:rounded-[2.5rem] border border-white dark:border-gray-700/50 text-center">
          ...
          {status === 'loading' && (
            <div className="flex flex-col items-center space-y-6">
              ...
              <div>
                <h2 className="text-2xl font-black text-gray-900 dark:text-white tracking-tight uppercase">{t('auth.verifyEmailLoading')}</h2>
                <p className="mt-2 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">{t('auth.processing') || 'Processing...'}</p>
              </div>
            </div>
          )}
          ...
          {status === 'success' && (
            <div className="flex flex-col items-center space-y-6 animate-in zoom-in-95 duration-500">
              ...
              <div>
                <h2 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight leading-tight">{t('auth.verifyEmailSuccess')}</h2>
                <p className="mt-3 text-sm font-bold text-gray-500 dark:text-gray-400 leading-relaxed px-4">{message}</p>
              </div>
          ...
          {status === 'error' && (
            <div className="flex flex-col items-center space-y-6 animate-in zoom-in-95 duration-500">
              ...
              <div>
                <h2 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight leading-tight">{t('auth.verifyEmailFail')}</h2>
                <p className="mt-3 text-sm font-bold text-gray-500 dark:text-gray-400 leading-relaxed px-4">{message}</p>
              </div>
              <div className="w-full pt-6 border-t border-gray-50 dark:border-gray-700/50 space-y-4">
                ...
                <Link to="/login" className="inline-flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-primary transition-all">
                  {t('auth.backToLogin')}
                </Link>
              </div>
            </div>
