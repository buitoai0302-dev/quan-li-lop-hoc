      ...
      <div className="flex flex-col gap-3 mb-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex flex-col sm:flex-row gap-2 flex-1 sm:max-w-xl order-2 sm:order-1">
            ...
            <select
              className="w-full sm:w-auto border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary dark:bg-gray-900 transition-all shadow-sm appearance-none bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20fill%3D%22none%22%20viewBox%3D%220%200%2024%2024%22%20stroke%3D%22%236B7280%22%3E%3Cpath%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%20stroke-width%3D%222%22%20d%3D%22M19%209l-7%207-7-7%22%2F%3E%3C%2Fsvg%3E')] bg-[length:16px] bg-[right_12px_center] bg-no-repeat pr-10"
              value={branchFilter}
              onChange={(e) => setBranchFilter(e.target.value)}
            >
              <option value="" className="dark:bg-gray-900">{t('import.selectBranch')}</option>
              {branches.map(b => <option key={b.id} value={b.id} className="dark:bg-gray-900">{b.name}</option>)}
            </select>
          </div>
          ...
          <div className="flex items-center justify-end gap-2 order-1 sm:order-2">
            ...
            >
              ...
              <span className="sm:inline">{t('teachers.addTeacher')}</span>
            </button>
          </div>
        </div>
      </div>
            ...
            <table className="w-full min-w-full divide-y divide-gray-200 dark:divide-gray-700 border-separate border-spacing-0">
              <thead className="sticky top-0 z-10">
                <tr className="bg-gray-50 dark:bg-gray-900/90 backdrop-blur-md">
                  <th className="w-auto px-2 sm:px-6 py-3 text-left text-[10px] sm:text-xs font-extrabold text-gray-500 dark:text-gray-400 uppercase tracking-wider border-b border-gray-200 dark:border-gray-700 whitespace-nowrap">{t('teachers.name')}</th>
                  ...
                  <th className="hidden sm:table-cell w-[20%] px-6 py-3 text-left text-xs font-extrabold text-gray-500 dark:text-gray-400 uppercase tracking-wider border-b border-gray-200 dark:border-gray-700 whitespace-nowrap">{t('teachers.specialization')}</th>
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {filteredTeachers.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((teacher) => (
                  <tr key={teacher.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                    <td className="px-2 sm:px-6 py-3">
                      <div className="flex items-center min-w-0">
                        <div className="relative flex-shrink-0">
                          <div className="h-9 w-9 bg-primary/10 dark:bg-primary/20 rounded-xl flex items-center justify-center text-primary font-bold text-[10px] sm:text-xs">
                            {teacher.full_name.substring(0, 2).toUpperCase()}
                          </div>
                          <div className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-white dark:border-gray-800 ${teacher.is_active ? 'bg-green-500' : 'bg-gray-400'}`}></div>
                        </div>
                        <div className="ml-4 sm:ml-5 min-w-0 flex-1">
                          <div className="text-sm font-bold text-gray-900 dark:text-gray-100 truncate">{teacher.full_name}</div>
                          <div className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 truncate">{teacher.email}</div>
                          <div className="text-[10px] text-primary sm:hidden font-medium truncate">
                    ...
                    <td className="hidden md:table-cell px-6 py-3 text-sm text-gray-900 dark:text-gray-300 truncate">
                      {teacher.branch_name || t('rooms.unknown')}
                    </td>
                    ...
                    <td className="px-2 sm:px-6 py-3 whitespace-nowrap text-right">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => handleOpenModal(teacher)} className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 transition-colors">
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                          </svg>
                        </button>
                        <button onClick={() => setDeletingId(teacher.id)} className="text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300 transition-colors">
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-1">
          ...
          <div className="bg-amber-50/50 dark:bg-amber-900/10 p-4 rounded-2xl border border-amber-100/50 dark:border-amber-500/10 space-y-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] font-black text-amber-600/50 dark:text-amber-400/50 uppercase tracking-widest">{t('common.info')}</span>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                <label className="block text-[11px] font-bold text-gray-700 dark:text-gray-300 mb-1 ml-1">{t('teachers.name')} *</label>
                <input
                  required
                  type="text"
              ...
              <div>
                <label className="block text-[11px] font-bold text-gray-700 dark:text-gray-300 mb-1 ml-1">{t('teachers.phone')}</label>
                <input
                  type="text"
                  placeholder="09xxx..."
                  className="block w-full bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 rounded-xl py-2.5 px-4 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-sm font-medium dark:text-white"
          ...
          <div className="bg-indigo-50/50 dark:bg-indigo-900/10 p-4 rounded-2xl border border-indigo-100/50 dark:border-indigo-500/10 space-y-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] font-black text-indigo-600/50 dark:text-indigo-400/50 uppercase tracking-widest">{t('teachers.professional')}</span>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                <label className="block text-[11px] font-bold text-gray-700 dark:text-gray-300 mb-1 ml-1">{t('teachers.email')} *</label>
                <input
                  required
                  type="email"
                  disabled={!!editingId}
                  placeholder="teacher@example.com"
                  className={`block w-full border-gray-200 dark:border-gray-700 rounded-xl py-2.5 px-4 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-sm font-medium ${editingId ? 'bg-gray-100 dark:bg-gray-700 text-gray-500' : 'bg-white dark:bg-gray-800 dark:text-white'}`}
                  value={formData.email}
                  onChange={(e) => setFormData({ ...formData, email: e.target.value })}
                />
              </div>
              <div className="sm:col-span-1">
                <label className="block text-[11px] font-bold text-gray-700 dark:text-gray-300 mb-1 ml-1">{t('teachers.specialization')}</label>
                <input
                  type="text"
