      ...
      <div className="space-y-8 max-w-6xl mx-auto pb-12">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-gray-700 overflow-hidden">
                    ...
                    <tr key={req.id} className="hover:bg-gray-50/80 dark:hover:bg-gray-700/20 transition-all group">
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-3">
                          <div className={`w-9 h-9 rounded-lg flex items-center justify-center shadow-sm ${req.plan_name.includes('Pro') ? 'bg-amber-50 text-amber-500 border border-amber-100' :
                        ...
                        {req.status === PLAN_REQUEST_STATUS.PENDING && (
                          <div className="flex justify-end gap-3">
                            <button
                              ...
                              className="flex items-center gap-2 px-5 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg font-black text-[10px] uppercase tracking-widest transition-all hover:scale-105 active:scale-95 shadow-lg shadow-emerald-500/30"
                            >
                            ...
                            <button
                              ...
                              className="flex items-center gap-2 px-5 py-2.5 bg-rose-500 hover:bg-rose-600 text-white rounded-lg font-black text-[10px] uppercase tracking-widest transition-all hover:scale-105 active:scale-95 shadow-lg shadow-rose-500/30"
                            >
          ...
          <div className="lg:hidden space-y-3 p-3">
            {requests.length === 0 ? (
              <div className="bg-white dark:bg-gray-800 p-10 text-center rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                <Clock size={40} className="mx-auto mb-3 text-gray-300" />
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">{t('common.noData')}</p>
              </div>
            ) : (
              requests.map((req) => (
                <div key={req.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-md border border-gray-100 dark:border-gray-700 space-y-3">
                  <div className="flex justify-between items-start gap-2">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-10 h-10 shrink-0 rounded-lg bg-gradient-to-br from-indigo-500 to-blue-600 flex items-center justify-center text-white shadow-md">
                        <Building size={20} />
                      </div>
                  ...
                  {req.status === PLAN_REQUEST_STATUS.PENDING && (
                    <div className="flex gap-2 pt-1">
                      <button
                        onClick={() => setConfirmModal({ isOpen: true, id: req.id, action: PLAN_REQUEST_ACTIONS.APPROVE })}
                        className="flex-1 py-2.5 bg-emerald-500 text-white rounded-lg font-black text-[10px] uppercase tracking-widest shadow-sm active:scale-95 transition-all"
                      >
                        {t('admin.approve')}
                      </button>
                      <button
                        onClick={() => setConfirmModal({ isOpen: true, id: req.id, action: PLAN_REQUEST_ACTIONS.REJECT })}
                        className="flex-1 py-2.5 bg-rose-500 text-white rounded-lg font-black text-[10px] uppercase tracking-widest shadow-sm active:scale-95 transition-all"
                      >
                        {t('admin.reject')}
                      </button>
                    </div>
