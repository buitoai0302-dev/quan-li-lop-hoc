    ...
    <div className="space-y-6 animate-in fade-in duration-700 pb-12">
      ...
      <div className="relative bg-gradient-to-br from-gray-900 to-indigo-950 rounded-[2.5rem] p-8 md:p-10 overflow-hidden shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6 text-white">
          ...
          <div className="flex items-center gap-6 p-5 bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 shadow-inner">
            <div className="text-center">
              <div className="text-primary-light text-3xl font-black">{stats?.overallAttendance || 0}%</div>
              <div className="text-white/40 text-[9px] font-black uppercase tracking-widest">{t('dashboard.attendanceRate')}</div>
            </div>
            <div className="w-px h-10 bg-white/10"></div>
            <div className="flex -space-x-2">
              {[1, 2, 3].map(i => (
                <div key={i} className="w-8 h-8 rounded-full border-2 border-indigo-950 bg-gray-700 overflow-hidden">
                  <img src={`https://i.pravatar.cc/100?u=${i}`} alt="user" />
                </div>
              ))}
              <div className="w-8 h-8 rounded-full border-2 border-indigo-950 bg-primary flex items-center justify-center text-[10px] font-bold">+12</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Main Content Area - Gated for Free Plan */}
